...
VerificationTest[
  Module[{worldId, floorId, cylId, positionsStart, positionsEnd},
    worldId = RapierWorldCreate[{0.0, 0.0, -9.81}];
    ...
    (* Cylinder at Z = 5.0, Dynamic *)
    cylId = RapierAddRigidBody[worldId, {0.0, 0.0, 5.0}, "Dynamic"];
    RapierAddColliderCylinder[worldId, cylId, {0.5, 0.25}, 1.0];
    ...
    (* Verify cylinder is falling (Z is decreasing) *)
    {positionsStart[[2, 4]], positionsEnd[[2, 4]], positionsStart[[2, 4]] > positionsEnd[[2, 4]]}
  ],
  {5.0, current_ /; current < 5.0, True},
  SameTest -> MatchQ,
  TestID -> "RapierDropCylinderZ-Decrease"
]
